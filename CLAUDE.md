...
**`cd` is and must remain a shell built-in**: `SYS_EXEC` saves the CWD before each child and
unconditionally restores it afterwards — any `chdir()` call inside a child binary would be
discarded on exit. This is the same reason `cd` is a built-in in all Unix shells.
...
| bin/t_exec.c     | t_exec     | spawn `/bin/hello` 300 times sequentially; verifies process slot recycling → "exec: OK" |
...
bin/t_exec.c           [test] spawn hello 300 times → "exec: OK"; process slot recycling
scripts/patch_boot.sh  splices boot code with BPB from mkfs.fat into sector 0
docs/gen_diagrams.py   generates architecture PNG diagrams via Pillow (physical-layout,
                       pcb-struct, interrupt-frame, process-frames, page-directory)
...
| t_exec            | `t_exec` spawns hello 300 times; all must succeed; prints "exec: OK" (timeout 90 s) |
...
- [x] `t_exec` stress test — 300× `exec("hello","")` in a loop; verifies PCB slot recycling
- [x] Architecture PNG diagrams in `docs/` (physical-layout, pcb-struct, interrupt-frame,
      process-frames, page-directory); generated by `docs/gen_diagrams.py` (Pillow)
- [x] Automated test suite (17 tests, pexpect + QEMU)
